...
bool crid_build_beacon_frame(cn_crid_config_t *config,
    ...
    if (max_len < BEACON_FRAME_ESTIMATED_LEN) {
        ESP_LOGE(TAG, "Frame buffer too small: %u < %u", (unsigned)max_len, (unsigned)BEACON_FRAME_ESTIMATED_LEN);
        return false;
    }
    ...
    // Sequence Control（12-bit 序号）
    // 用报文计数器派生递增序号：配合 en_sys_seq=false 注入时，驱动直接使用帧内序号；
    // 若固定为 0，稀疏发送（如仅 1 架无人机、每秒 1 帧）时可能被芯片/接收机
    // 判为重复/无效帧而丢弃，表现为“数量少时收不到”。
    // 每架无人机各有 message_counter，故每架序号独立递增。
    uint16_t seq_ctrl = (uint16_t)(config->message_counter & 0x0FFF);
    frame[pos++] = (uint8_t)(seq_ctrl & 0xFF);
    frame[pos++] = (uint8_t)((seq_ctrl >> 8) & 0x0F);
    ...
    *out_len = pos;
    ...
    // 每 20 帧打印一次，避免 3 架无人机每秒 3 条日志刷屏挤占单核 CPU
    if ((msg_counter % 20) == 0) {
        ESP_LOGI(TAG, "Beacon frame built: %u bytes, counter=%u, pos=(%.6f,%.6f)",
                 (unsigned)*out_len, (unsigned)msg_counter, config->latitude, config->longitude);
    }
    return true;
}
